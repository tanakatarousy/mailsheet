...
test("lets testers submit questions and admins manage their status", async () => {
  ...
  assert.match(worker, /handleAdminFeedbackAttachments/);
  assert.match(worker, /FEEDBACK_FILES\.put/);
  assert.match(worker, /MAX_FEEDBACK_ATTACHMENTS = 3/);
  ...
  assert.match(worker, /\/api\/admin\/feedback\/attachment/);
  ...
  assert.match(styles, /\.tester-feedback-attachments/);
  assert.match(styles, /\.admin-feedback-attachments/);
